...
- 留意点1: 1階層ルールを守り続ける。環境が増えても(`bihin-stg` 等)第1階層に置く限り1枚で足りる
- 留意点2: 検証と本番が同じ秘密鍵を共有することになる。App Serviceでは鍵をエクスポート不可のままプラットフォーム管理にできるため実務リスクは小さいが、検証環境の統制レベルを下げる運用(外部委託者の利用等)を始める場合は、環境別に親サブドメインと証明書を分ける(費用は2枚分)
